    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea
            placeholder="Share your idea... (e.g. A post about the importance of sleep)"
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            className="w-full p-6 bg-white border border-slate-300 rounded-3xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40"
          />
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            <button
              className="flex items-center gap-3 bg-purple-50 py-2 px-3 rounded-lg"
              onClick={() => setGenerateImage(!generateImage)}
            >
              <span>AI Image</span>
              <div
                className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${
                  generateImage ? "bg-purple-600" : "bg-slate-200"
                }`}
              >
                <span
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {tones.map((t) => (
            <button
              key={t}
              onClick={() => setTone(t)}
              className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "border-purple-600 bg-purple-600 text-white" : "bg-white border-slate-200 text-slate-500 hover:border-slate-300"}`}
            >
              {t}
            </button>
          ))}
      ...
      <div className="space-y-6 pt-12 border-t border-slate-100">
        <div className="flex items-center justify-between text-slate-600">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl text-slate-700">Recent Generations</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {generations.map((gen) => (
            <div
              key={gen._id}
              className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-purple-200 transition-all relative overflow-hidden"
            >
              <div className="flex flex-col h-full space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-purple-600 bg-purple-50 px-2 py-0.5 rounded-md">
                    {gen.tone}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    ...
                    className="flex-1 bg-slate-100 hover:bg-purple-600 hover:text-white text-slate-600 text-xs py-2.5 rounded-lg transition-all"
                  >
                    Schedule Post
                  </button>
                </div>
          ...
          {generations.length === 0 && (
            <div className="col-span-full py-20 text-center space-y-2">
              ...
              <p className="text-slate-400 text-sm">
                No Content Generated Yet. Try generating some content by
                entering a prompt above.
              </p>
            </div>
          )}
        </div>
      ...
      {activeScheduler && (
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-8 py-4 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
                    ...
                    {PLATFORMS.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-2.5 rounded-md border text-xs ${active ? "bg-purple-600 text-white" : "bg-white border-slate-200 text-slate-400 hover:border-purple-300"}`}
                        >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-purple-600 hover:text-white transition"
              >
                {scheduling ? (
                  <Loader2Icon className="size-4 animate-spin" />
                ) : (
