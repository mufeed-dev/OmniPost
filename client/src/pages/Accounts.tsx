...
const Accounts = () => {
  ...
  useEffect(() => {
    ...
    const scyncNeeded = params.get("sync") === "true";
    ...
    if (connectedPlatform) {
      ...
      toast.error(`Account sync failed: ${decodeURIComponent(errorMsg)}`);
      fetchAccounts();
    } else if (scyncNeeded) {
  ...
  const handleConnect = async (platformId: string) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || `Failed to connect ${platformId}`,
      );
      setConnecting(null);
    }
  };
